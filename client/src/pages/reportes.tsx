                ...
                <Card className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white border-none shadow-lg">
                  <CardHeader className="pb-2">
                    <CardTitle className="flex items-center gap-2 text-white">
                      ...
                      Ganancia Neta Total (Últimos 30 días)
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="text-4xl font-bold font-mono" data-testid="text-total-profit">
                      {formatCurrency(advancedStats.stores.reduce((acc, s) => acc + s.profit, 0))}
                    </div>
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        <p className="text-emerald-100 text-sm">Total Ventas</p>
                        <p className="text-xl font-mono font-bold">
                          {formatCurrency(advancedStats.stores.reduce((acc, s) => acc + s.total_sales, 0))}
                        </p>
                      </div>
                      <div>
                        <p className="text-emerald-100 text-sm">Total Egresos</p>
                        <p className="text-xl font-mono font-bold">
                          {formatCurrency(advancedStats.stores.reduce((acc, s) => acc + s.total_expenses, 0))}
                        </p>
                      </div>
                    </div>
                    <p className="text-xs text-emerald-200 mt-2">
                      Fórmula: Ventas - Costos de Productos - Egresos
                    </p>
                  </CardContent>
                </Card>
